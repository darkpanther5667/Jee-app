...
export default function SubscriptionPage() {
  ...
  const handleCheckout = async (plan: 'pro' | 'elite') => {
    ...
    if (rzpKey) {
      // ─── FIX #11 / #20: Real Razorpay flow — server verifies signature ────────
      // Step 1: Create an order on the server first (needed for signature verification)
      ...
      const options = {
        ...
        // Step 2: On success, send all three IDs to server for HMAC verification
  ...
  // ─── FIX #11: Sandbox upgrade calls sandboxUpgradeSubscriptionAction ─────────
  // That action checks server-side that no real Razorpay key is configured.
...
export default function SubscriptionPage() {
  ...
  // Interactive Razorpay Sandbox Simulator states
  const [showSimulatorModal, setShowSimulatorModal] = useState(false)
  const [paymentMethod, setPaymentMethod] = useState<'upi' | 'card' | 'netbanking'>('upi')
  const [upiId, setUpiId] = useState('')
  const [cardNumber, setCardNumber] = useState('')
  ...
  const handleCheckout = async (plan: 'pro' | 'elite') => {
    ...
    const rzpKey = process.env.NEXT_PUBLIC_RAZORPAY_KEY_ID
    if (rzpKey) {
      // 1. Live/Test Razorpay Checkout Flow
      const amountPaise = plan === 'pro' ? 99900 : 199900
      const options = {
        key: rzpKey,
        amount: amountPaise,
        ...
        handler: async function () {
          setIsProcessing(true)
          try {
            const result = await updateSubscriptionTierAction(plan)
            if (result.success) {
              setPaymentSuccess(true)
              setCurrentTier(plan)
              setTimeout(() => {
      ...
      try {
        ...
        setErrorMsg('Failed to initialize Razorpay checkout. Falling back to test checkout...')
        setShowSimulatorModal(true)
      }
    } else {
      // 2. Immersive Test/Sandbox Checkout Simulator
      setShowSimulatorModal(true)
    }
  }
  ...
  const handleSimulatedPayment = async () => {
    ...
    // Simulate payment authorization delay to replicate live network round-trip
    await new Promise(resolve => setTimeout(resolve, 1800))

    try {
      const result = await updateSubscriptionTierAction(selectedPlan)
      if (result.success) {
        setPaymentSuccess(true)
        setCurrentTier(selectedPlan)
    ...
    <div className="min-h-screen bg-slate-50 font-sans pb-24 relative overflow-hidden">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-[10px] font-bold text-neutral-desc uppercase tracking-wider">
              Choose your tailored learning acceleration plan
            </p>
          </div>
        </div>
        <div className="px-3 py-1 bg-slate-100 text-slate-700 text-[10px] font-extrabold uppercase rounded-full border border-slate-200">
      ...
      <main className="max-w-5xl w-full mx-auto px-6 pt-12 space-y-10 relative z-10">
        ...
        <div className="text-center space-y-3 max-w-xl mx-auto">
          ...
          <p className="text-xs text-neutral-desc leading-relaxed">
            Gain unlimited access to National exam simulations, KaTeX step-by-step explainer key sheets, interactive scatter analytics, and educator priority doubts support.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 pt-4">
          ...
          <div className={`bg-white rounded-3xl border border-slate-200 p-6 flex flex-col justify-between shadow-sm relative overflow-hidden transition-all duration-300 ${currentTier === 'free' ? 'ring-2 ring-slate-400' : ''}`}>
            {currentTier === 'free' && (
              <span className="absolute top-4 right-4 px-2.5 py-0.5 bg-slate-100 text-slate-700 text-[9px] font-extrabold uppercase rounded-full border border-slate-200">
                ACTIVE
              </span>
            )}
            <div>
              ...
              <ul className="space-y-3.5">
                {[
                  '3 Full Mock Exam Papers',
                  'PYQ Browser Index queries',
                  'Standard chapter-wise filters',
                  'Basic score evaluations'
                ].map((feat, idx) => (
                  <li key={idx} className="flex items-start gap-2.5 text-xs text-slate-600">
            ...
            <div className="pt-8">
              <button
                disabled
                className="w-full py-3 bg-slate-100 text-slate-500 font-bold rounded-xl text-xs transition-all uppercase tracking-wider"
              >
                {currentTier === 'free' ? 'Currently Active' : 'Free Standard Account'}
              </button>
            </div>
          ...
          {/* PLAN 2: PRO TIER (POPULAR) */}
          <div className={`bg-white rounded-3xl border border-slate-200 p-6 flex flex-col justify-between shadow-sm relative overflow-hidden transition-all duration-300 ${currentTier === 'pro' ? 'ring-2 ring-primary border-primary/20' : 'hover:border-primary/20 hover:shadow-md'}`}>
            <div className="absolute top-0 right-0 w-36 h-36 bg-primary/5 rounded-full blur-2xl pointer-events-none -translate-y-1/2 translate-x-1/2" />
            
            {currentTier === 'pro' ? (
              <span className="absolute top-4 right-4 px-2.5 py-0.5 bg-primary/10 text-primary text-[9px] font-extrabold uppercase rounded-full border border-primary/20">
                ACTIVE
              </span>
            ) : (
              <span className="absolute top-4 right-4 px-2.5 py-0.5 bg-primary text-white text-[9px] font-black uppercase rounded-full shadow-sm flex items-center gap-1">
            ...
            <div>
              <h3 className="font-display font-black text-xl text-neutral-title uppercase tracking-wide flex items-center gap-1.5">
                <span>Pro Tier</span>
              </h3>
              <p className="text-[10px] text-neutral-desc mt-1.5 leading-relaxed">The ultimate pack for class 11, 12, & droppers</p>
              ...
              <ul className="space-y-3.5">
                {[
                  'Unlimited Mock Tests (Scheduled + Dynamic)',
                  'Full Step-by-Step solutions & video walk sheets',
                  'All PYQ detailed explanations unlocked',
                  'Advanced Response Time scatter charts',
                  'Est. National Rank & Percentile predictor',
                  'Custom Bookmarks + revision tags'
                ].map((feat, idx) => (
                  <li key={idx} className="flex items-start gap-2.5 text-xs text-slate-600">
                    <Check className="h-4 w-4 text-emerald-500 shrink-0 mt-0.5" />
          ...
          <div className={`bg-white rounded-3xl border border-slate-200 p-6 flex flex-col justify-between shadow-sm relative overflow-hidden transition-all duration-300 ${currentTier === 'elite' ? 'ring-2 ring-purple-500 border-purple-500/20' : 'hover:border-purple-300 hover:shadow-md'}`}>
            ...
            {currentTier === 'elite' && (
              <span className="absolute top-4 right-4 px-2.5 py-0.5 bg-purple-100 text-purple-700 text-[9px] font-extrabold uppercase rounded-full border border-purple-200">
                ACTIVE
              </span>
            )}
            ...
            <div>
              ...
              <ul className="space-y-3.5">
                {[
                  ...
                  'Guaranteed educator doubt replies (< 4 hours)',
                  'Personalized Spaced Repetition Revision Scheduler',
                  'Weak chapter accuracy target builders',
                  'Downloadable test result analysis reports (PDF)',
                  'Early access to all phase releases'
                ].map((feat, idx) => (
                  <li key={idx} className="flex items-start gap-2.5 text-xs text-slate-600">
                    <Check className="h-4 w-4 text-emerald-500 shrink-0 mt-0.5" />
                    <span>{feat}</span>
        ...
        <div className="max-w-md mx-auto text-center p-4 bg-slate-100 rounded-2xl border border-slate-200 flex items-center justify-center gap-3">
          <Shield className="h-5 w-5 text-slate-400" />
          <span className="text-[10px] font-bold text-neutral-desc uppercase tracking-wider">
            Secured checkout via Indian Unified Payments Interface (UPI) & Cards.
          </span>
        </div>

      </main>

      {/* 4. TRANSACTION PROCESS MODAL */}
      {isProcessing && (
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl p-8 max-w-sm w-full border border-slate-200 shadow-premium flex flex-col items-center text-center space-y-4 animate-scale-up">
            <Loader2 className="h-10 w-10 text-primary animate-spin" />
            <div>
              <h4 className="font-display font-extrabold text-base text-neutral-title">Contacting Razorpay Gateway</h4>
              <p className="text-[11px] text-neutral-desc mt-1">Please do not refresh or hit the back button. Initializing secure transaction channel...</p>
            </div>
          </div>
        </div>
      )}
      ...
      {paymentSuccess && (
        <div className="fixed inset-0 z-50 bg-slate-900/80 backdrop-blur-md flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl p-10 max-w-sm w-full border border-slate-200 shadow-premium flex flex-col items-center text-center space-y-4 animate-scale-up">
            <div className="h-16 w-16 bg-emerald-100 rounded-full flex items-center justify-center text-emerald-600 mb-2">
              <Check className="h-8 w-8 stroke-[3]" />
            </div>
            <div>
              <h4 className="font-display font-black text-xl text-neutral-title uppercase tracking-wide">Payment Confirmed</h4>
              <p className="text-xs text-neutral-desc mt-2">
                Congratulations! Your profile has been upgraded to <strong className="text-primary uppercase font-extrabold">{currentUser?.profile?.subscription_tier || selectedPlan}</strong> status. Accelerating mock test engine...
              </p>
            </div>
          </div>
        </div>
      ...
      {/* 7. PREMIUM INTERACTIVE RAZORPAY SANDBOX SIMULATOR MODAL */}
      {showSimulatorModal && (
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl w-full max-w-md border border-slate-200 shadow-premium overflow-hidden animate-scale-up relative">
            ...
            <div className="bg-slate-950 text-white p-6 relative">
              <button 
                ...
                className="absolute top-4 right-4 text-slate-400 hover:text-white transition-colors text-sm font-bold p-1 bg-slate-800 rounded-full"
              >
                ✕
              </button>
              <div className="flex items-center gap-1.5">
                <span className="px-2 py-0.5 bg-amber-500 text-amber-950 text-[9px] font-black rounded-full uppercase tracking-wider">
                  Razorpay Sandbox
                </span>
                <span className="px-2 py-0.5 bg-primary text-white text-[9px] font-black rounded-full uppercase tracking-wider">
                  Test Account
                </span>
              </div>
              <div className="flex justify-between items-end mt-4">
                ...
                <div className="text-right">
                  <span className="text-[9px] text-slate-400 block font-bold uppercase tracking-wider">amount</span>
                  <span className="font-display font-black text-xl text-amber-400">₹{selectedPlan === 'pro' ? '999' : '1,999'}</span>
                </div>
              </div>
            </div>
            ...
            <div className="p-6 space-y-4 min-h-[200px] flex flex-col justify-between">
              {paymentMethod === 'upi' ? (
                <div className="space-y-3">
                  <label className="block text-[10px] font-bold text-slate-400 uppercase">Enter Virtual Payment Address (VPA)</label>
                  <input
                    type="text"
                    placeholder="aspirant@okaxis"
                    value={upiId}
                    onChange={(e) => setUpiId(e.target.value)}
                    className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold text-slate-700 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:bg-white"
                  />
                  <p className="text-[9px] text-slate-400 italic">Enter any mock address to authorize instantly.</p>
                </div>
              ) : paymentMethod === 'card' ? (
                <div className="space-y-3">
                ...
                <div className="space-y-3">
                  <label className="block text-[10px] font-bold text-slate-400 uppercase">Select Popular Indian Bank</label>
                  <select
                    value={selectedBank}
              ...
              <button
                ...
                className="w-full py-3 bg-primary hover:bg-primary-dark active:scale-[0.98] text-white font-bold rounded-xl text-xs uppercase tracking-wider transition-all mt-4"
              >
            ...
            {/* Footer */}
            <div className="bg-slate-50 px-6 py-3.5 border-t border-slate-100 flex items-center justify-center gap-2 text-[9px] font-bold text-slate-400 uppercase">
              <Shield className="h-4 w-4" /> SECURED INTEGRATED UPI GATEWAY
            </div>
